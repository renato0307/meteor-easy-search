EasySearch.getComponentInstance = (function () {
  ...
  Template.esPagination.helpers({
    ...
    'control' : function () {
      ...
      controls = [{ text: '«', state: 'disabled', action: EasySearch.PAGINATION_PREV }];

      if (skip > 0) {
        controls[0].state = 'canbeactive';
      }

      for (i = 1; i <= sites; i += 1) {
        control = { text: i.toString(), state: 'canbeactive', action: i };
      ...
      control = { text: '»', state: 'canbeactive', action: EasySearch.PAGINATION_NEXT };
